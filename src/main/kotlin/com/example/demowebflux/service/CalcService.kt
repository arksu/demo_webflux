...
class CalcService(
    ...
    fun calcOrderAmounts(invoice: Invoice, referenceAmount: BigDecimal, commission: BigDecimal): CalcModel {
        ...
        return when (invoice.commissionType) {
            CommissionType.CLIENT -> {
                CalcModel(
                    ...
                    // сколько отдаем мерчанту в валюте сделки с учетом комиссий
                    merchantAmountByOrder = referenceAmount,
                    // сколько отдаем мерчанту в валюте invoice
                    merchantAmountByInvoice = invoice.amount
                )
            }
            ...
            CommissionType.MERCHANT -> {
                CalcModel(
                    ...
                    customerAmount = referenceAmount,
                    // сколько отдаем мерчанту в валюте сделки с учетом комиссий
